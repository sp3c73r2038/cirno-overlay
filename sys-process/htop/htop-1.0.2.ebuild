...
DESCRIPTION="interactive process viewer, solarized color and vi patching"
...
IUSE="kernel_FreeBSD kernel_linux openvz solarized-color unicode vi vserver"
...
src_prepare() {
  ...
  if use solarized-color; then
	epatch "${FILESDIR}/${P}-solarized.patch"
  fi

  if use vi; then
	epatch "${FILESDIR}/${P}-vi.patch"
  fi
...
src_configure() {
  [[ $CBUILD != $CHOST ]] && export ac_cv_file__proc__{meminfo,stat}=yes #328971
