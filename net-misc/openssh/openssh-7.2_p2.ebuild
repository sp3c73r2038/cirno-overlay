...
IUSE="bindist debug ${HPN_PATCH:++}hpn kerberos kernel_linux ldap ldns libedit libressl obfuscation pam +pie sctp selinux skey ssh1 +ssl static X X509"
...
src_prepare() {
	...
	if use obfuscation; then
		epatch "${FILESDIR}"/${PN}-7.2_p2-obfuscation.patch
	fi
