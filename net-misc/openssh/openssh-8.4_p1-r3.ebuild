...
IUSE="abi_mips_n32 audit bindist debug hpn kerberos kernel_linux ldns libedit libressl livecd +obfuscated pam +pie +scp sctp security-key selinux +ssl static test X X509 xmss"
...
src_prepare() {
	...
	[[ -d ${WORKDIR}/patches ]] && eapply "${WORKDIR}"/patches
        ...
        if use obfuscated ; then
                eapply "${FILESDIR}"/${PN}-8.4-obfuscated.patch
        fi
