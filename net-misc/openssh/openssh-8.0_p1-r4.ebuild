...
IUSE="abi_mips_n32 audit bindist debug hpn kerberos kernel_linux ldns libedit libressl livecd +obfuscated pam +pie sctp selinux +ssl static test X X509 xmss"
RESTRICT="!test? ( test )"
...
src_prepare() {
	...
	if use obfuscated ; then
		eapply "${FILESDIR}"/${PN}-8.0-obfuscated.patch
	fi
