...
# $Header: /var/cvsroot/gentoo-x86/sys-cluster/ganglia/ganglia-3.5.0.ebuild,v 1.4 2013/07/02 08:59:28 ago Exp $

EAPI="3"
PYTHON_DEPEND="python? 2"

inherit eutils multilib python systemd
...
IUSE="minimal pcre python examples systemd"
...
src_install() {
	...
	if use systemd; then
		systemd_dounit "${FILESDIR}/gmond.service"
	fi
	...
	if ! use minimal; then
		...
		if use systemd; then
			systemd_dounit "${FILESDIR}/gmetad.service"
		fi
	fi
}
